...
public:
  struct Pixel
  {
    ...
    Pixel(unsigned index, unsigned width) : x(index % width), y(index / width)
    {
    }
    int x;
    int y;
...
private:
  ...
  RosMap();
  static RosMap::KdTreeNode* generateKdTree(std::vector<Pixel>& pixels, int begin,
                                            int end, bool split_x = true);
  static bool comparePixelsByX(const Pixel& pixel0, const Pixel& pixel1)
  {
    return pixel0.x < pixel1.x;
  }
  static bool comparePixelsByY(const Pixel& pixel0, const Pixel& pixel1)
  {
    return pixel0.y < pixel1.y;
  }
  static void searchNearestNeighbour(const KdTreeNode* node, const Pixel& pixel,
                                     Pixel& best, double& distance, bool split_x = true);
...
inline void RosMap::computeDistances()
{
...
inline void RosMap::pixelToMap(const Pixel& pixel, utils::Vector2d& x) const
{
...
inline void RosMap::mapToPixel(const utils::Vector2d& x, Pixel& pixel) const
{
  double mapX = x.getX();
  double mapY = x.getY();
...
inline bool RosMap::isObstacle(const utils::Vector2d& x) const
{
  Pixel pixel;
  mapToPixel(x, pixel);
...
inline const Map::Obstacle& RosMap::getNearestObstacle(const utils::Vector2d& x)
{
  Pixel pixel;
  mapToPixel(x, pixel);
...
inline RosMap::KdTreeNode* RosMap::generateKdTree(std::vector<Pixel>& pixels, int begin,
                                                  int end, bool split_x)
{
  if (begin == end)
  {
    return NULL;
...
inline void RosMap::searchNearestNeighbour(const KdTreeNode* node, const Pixel& pixel,
                                           Pixel& best, double& distance, bool split_x)
{
  KdTreeNode *next, *other;
  double current_distance = pixel.distance(node->pixel);

  if ((split_x && comparePixelsByX(pixel, node->pixel)) ||
      (!split_x && comparePixelsByY(pixel, node->pixel)))
  {
    next = node->left;
    other = node->right;
  }
  ...
  else
  {
    ...
    int distance_to_plane = split_x ? (pixel.x - node->pixel.x) * (pixel.x - node->pixel.x) :
                                      (pixel.y - node->pixel.y) * (pixel.y - node->pixel.y);
