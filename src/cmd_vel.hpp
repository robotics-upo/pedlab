...
{
public:
  CmdVelProvider(double obstacle_distance_threshold, double robot_max_lin_vel,
                 double robot_max_lin_acc, double robot_max_ang_acc, double beta_v,
                 double beta_y, double beta_d);
  ~CmdVelProvider()
  {
  }
  bool compute(const Agent& robot, const Agent& target, bool targetFound, bool validGoal,
               double dt);
  const geometry_msgs::Twist& getCommand() const
  {
    return command;
  }
...
#define PW(x) ((x) * (x))
  ...
  bool checkCollision(const Agent& robot, double linearVelocity, double angularVelocity,
                      double& distance, double& time) const;

  double evaluate(double linVel, double angVel, double distance, double dt, const Agent& robot,
...
inline CmdVelProvider::CmdVelProvider(double obstacle_distance_threshold, double robot_max_lin_vel,
                                      double robot_max_lin_acc, double robot_max_ang_acc,
                                      double beta_v, double beta_y, double beta_d)
  ...
  , beta_d(beta_d)
{
  ...
  static const std::vector<double> lin_vels = { 0,    0.05, 0.1,  0.15, 0.2,  0.25, 0.3,
...
inline double CmdVelProvider::evaluate(double linVel, double angVel, double distance, double dt,
                                       const Agent& robot, const utils::Vector2d& velocityRef,
                                       const utils::Angle& yawRef)
{
...
inline bool CmdVelProvider::checkCollision(const Agent& robot, double linearVelocity,
                                           double angularVelocity, double& distance,
                                           double& time) const
{
  ...
  if (fabs(angularVelocity) < 1e-7)
  {
    ...
    for (unsigned k = 0; k < 2; k++)
    {
      ...
      {
        if ((linearVelocity > 0 && obstacles[i].getX() < 0) ||
            (linearVelocity < 0 && obstacles[i].getX() > 0))
        {
          continue;
        }
        if (fabs(obstacles[i].getY()) <= robot.radius)
        {
          ...
          coll = coll ||
                 ((aux_time >= 0.0) &&
                  fabs(linearVelocity) > std::sqrt(2 * aux_distance * robot_max_lin_acc));
  ...
  // Alternative version:
  utils::Vector2d icc(0, linearVelocity / angularVelocity);  // Instantaneous Centre of
                                                             // Rotation. In the local
                                                             // frame (base_link), it is
  ...
  for (unsigned k = 0; k < 2; k++)
  {
    ...
    for (unsigned i = 0; i < obstacles.size(); i++)
    {
      ...
      // If the obstacle is outside the annular disc traversed by the robot (radii a and
      // b, centered at icc), no collision
      ...
      double ctheta = (icc.squaredNorm() + u.squaredNorm() - PW(robot.radius)) /
                      (2.0 * icc.norm() * u.norm());
      ...
      // Angular distance until the collision (the current angle of the obstacle minus the
      // angle where the collision occurs)
      ...
      double aux_distance = deltaTheta * u.norm();  // Distance until collision. Angle (in
                                                    // radians) times radius. It has sign
      double aux_time = aux_distance / fabs(linearVelocity);


      // double aux_time = deltaTheta/fabs(angularVelocity);
      // double aux_distance = fabs(linearVelocity)*aux_time;
      // Check the condition on angular velocities. Can we avoid the obstacle by stopping
      // the rotation?
      ...
      coll =
          coll ||
          ((aux_time >= 0.0) &&
           ((fabs(linearVelocity) > std::sqrt(2 * fabs(aux_distance) * robot_max_lin_acc)) ||
            (fabs(angularVelocity) > std::sqrt(2 * fabs(deltaTheta) * robot_max_ang_acc))));

      // coll = coll || ((aux_time >= 0.0) && (fabs(linearVelocity) >
      // std::sqrt(2*fabs(aux_distance) * params.robotMaxLinearAcceleration)));
...
inline bool CmdVelProvider::compute(const Agent& robot, const Agent& target,
                                    bool targetFound, bool validGoal, double dt)
{
  ...
  for (unsigned i = 0; i < velocities.size(); i++)
  {
    ...
    markers.markers[i].points.assign(full_markers.markers[i].points.begin(),
                                     full_markers.markers[i].points.begin() + size);
    markers.markers[i].color.a = 1.0;
    markers.markers[i].scale.x = 0.01;

    if (fabs(linAcc) > robot_max_lin_acc || fabs(angAcc) > robot_max_ang_acc)
    {
