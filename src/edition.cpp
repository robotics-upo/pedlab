...
void EditionNode::saveFile()
{
  ...
  {
    stream << "\t<waypoint id=\"" << (it->first) << "\" x=\"" << (it->second.getX())
           << "\" y=\"" << (it->second.getY()) << "\" r=\"0.5\"/>\n";
  }
  stream << "\t<!-- edges -->\n";
...
inline void EditionNode::pointReceived(const geometry_msgs::PointStamped::ConstPtr& point)
{
  utils::Vector2d v(point->point.x, point->point.y);
  // To delete nodes, click first on the first cell of the map and later on the node to
  // delete
  ...
  while (it != nodes.end())
  {
    if (((it->second - v).norm() < distance) && (it->first != counter - 1))
    {
      if (delete_node)
      {
        ...
        while (it1 != edges.end())
        {
          if ((it->second - nodes.at(it1->src)).norm() < distance ||
              (it->second - nodes.at(it1->dst)).norm() < distance)
          {
            it1 = edges.erase(it1);
          }
          else
...
inline EditionNode::EditionNode(ros::NodeHandle& n, ros::NodeHandle& pn)
  : pn(pn), clicked(NULL), delete_node(false), counter(0)
{
  double freq;
  ...
  ros::Subscriber point_sub = n.subscribe<geometry_msgs::PointStamped>(
      "/clicked_point", 1, &EditionNode::pointReceived, this);
