...
private:
  ...
  bool transformPose(double& x, double& y, double& theta, const std::string& sourceFrameId,
                     const std::string& targetFrameId) const;
  bool transformPoint(double& x, double& y, const std::string& sourceFrameId,
                      const std::string& targetFrameId) const;
  bool transformVelocity(double& vx, double& vy, const std::string& sourceFrameId,
                         const std::string& targetFrameId) const;
...
inline void Node::init(TiXmlNode* pParent, std::vector<AgentCfg>& agents, int& peopleCount)
{
  if (!pParent)
    return;
...
inline bool Node::transformPose(double& x, double& y, double& theta,
  ...
  : gen(std::chrono::system_clock::now().time_since_epoch().count())
{
  ...
  people_markers_pub =
      pn.advertise<animated_marker_msgs::AnimatedMarkerArray>("/plab/markers/people", 1);
  ...
  {
    cmd_vel_target_sub = n.subscribe<geometry_msgs::Twist>(cmd_vel_target_id, 1,
                                                           &Node::targetCmdVelReceived, this);
  }
...
inline int Node::getPersonCollisionIndex(const utils::Vector2d& x, double collisionThreshold) const
{
  double d;
...
inline void Node::publishScan360(const ros::Time& current_time)
{
  utils::Vector2d x, u;
  ...
  {
    u.set(std::cos(angle) * sfm::MAP.getInfo().resolution,
          std::sin(angle) * sfm::MAP.getInfo().resolution);
    x = agents[0].position;
    d = 0;
    collisionIndex = -1;
...
inline bool Node::publishDetections(const ros::Time& current_time)
{
...
inline bool Node::publishPeople(const ros::Time& current_time)
{
...
inline bool Node::publishOdom(const ros::Time& current_time)
{
...
inline void Node::robotCmdVelReceived(const geometry_msgs::Twist::ConstPtr& cmd_vel)
{
  agents[0].linearVelocity = cmd_vel->linear.x;
  agents[0].angularVelocity = cmd_vel->angular.z;
}

inline void Node::targetCmdVelReceived(const geometry_msgs::Twist::ConstPtr& cmd_vel)
{
